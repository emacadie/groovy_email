...
class DATACommandSpec extends Specification {
	...
	def "test handling wrong command"() {
	    when:
	        def resultMap = dataCommand.process( "MAIL FROM:<oneill@stargate.mil>", [ 'RCPT' ] as Set, [ forwardPath:  [ hamilton ] ] )
	...
	*/
	...
	def "#inputAddress gives #value"() {
        ...
        when:
            resultMap = dataCommand.process( "RCPT TO:<${inputAddress}>", resultSetEM, [ forwardPath:  [ hamilton ] ] )
        then:
	...
	*/
	...
	def "#inputAddress with RCPT in previous command list does not add another RCPT and gives #value"() {
        ...
        when:
            resultMap = dataCommand.process( "RCPT TO:<${inputAddress}>", resultSetEMR, [ forwardPath:  hamilton ] )
        then:
            println "command was EHLO, resultString is ${resultMap.resultString}"
            resultMap.resultString == value
            resultMap.prevCommandSet == resultSetEMR
	...
	*/
	...
	def "#inputAddress with prev command sequence gives #value"() {
        ...
        when:
            resultMap = dataCommand.process( "RCPT TO:<${inputAddress}>", prevCommandSet, [ forwardPath: [ hamilton ] ] )
        then:
            println "command was EHLO, resultString is ${resultMap.resultString}"
            resultMap.resultString == value
            resultMap.prevCommandSet == resultSetEMR
	...
	*/
	...
	def "#inputAddress with wrong prev command sequence gives #value"() {
        ...
        when:
            resultMap = dataCommand.process( "RCPT TO:<${inputAddress}>", prevCommandSet, [:] )
        then:
            println "command was EHLO, resultString is ${resultMap.resultString}"
            resultMap.resultString == value
	...
	*/
	...
	def "test happy path"() {
	    when:
	        def resultMap = dataCommand.process( "RCPT TO:<${jackShell}>", resultSetEM, [ forwardPath:  [ hamilton ] ] )
	        def mailResponse = resultMap.resultString + crlf 
