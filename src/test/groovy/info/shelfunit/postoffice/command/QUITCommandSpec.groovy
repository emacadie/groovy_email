...
class QUITCommandSpec extends Specification {
	...
	def "test individual messages"() {
        ...
        sleep( 2 * 1000 )
        when:
            def resultMap = quitCommand.process( 'QUIT 1', [] as Set, bufferInputMap )
        then:
        ...
        when:
            resultMap = quitCommand.process( 'QUIT 3', [] as Set, bufferInputMap )
        then:
            resultMap.bufferMap.totalMessageSize == totalMessageSizeTest
        ...
        when:
            resultMap = quitCommand.process( 'QUIT 2', [] as Set, bufferInputMap )
        then:
            resultMap.bufferMap.totalMessageSize == totalMessageSizeTest
        ...
        when:
            resultMap = quitCommand.process( 'QUIT 4', [] as Set, bufferInputMap )
        then:
            resultMap.bufferMap.totalMessageSize == totalMessageSizeTest
            resultMap.resultString == "-ERR no such message, only 3 messages in maildrop"
            
        when:
            resultMap = quitCommand.process( 'QUIT 1', [] as Set, bufferInputMap )
        then:
            resultMap.bufferMap.totalMessageSize == totalMessageSizeTest
